...
def adhaar_read(text):
    ...
    for i, data in enumerate(text0):
        ...
        if 'DOB' in data or 'DO8' in data:
            ...
            name = text0[l - 1]
            # print(name)
        elif 'MALE' in data or 'FEMALE' in data:
            gen = data
    ...
    try:
        ...
        address = address.replace("\"", "")
        address = address.replace(",,", ",")
        # replace anything before S/O, W/O, D/O with ""
        address = re.sub(".*S/O |.*s/o", "S/O ", address)
        address = re.sub(".*W/O |.*w/o", "W/O ", address)
        address = re.sub(".*D/O |.*d/o ", "D/O ", address)
        address = re.sub(".*C/O |.*c/o ", "C/O ", address)
        address = re.sub(".*H/O |.*h/o ", "H/O ", address)

        # replace , in the end with .
        ...
        code = code.replace("[", " ")
        code = code.replace("]", " ")
        # replace anything other than 6 digits with ""
